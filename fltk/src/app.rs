...
pub fn add_system_handler<F: FnMut(Event) -> bool + 'static>(cb: F) {
    unsafe {
        unsafe extern "C" fn shim(ev: raw::c_int, data: *mut raw::c_void) -> raw::c_int {
            let a: *mut Box<dyn FnMut(raw::c_int) -> bool> =
                data as *mut Box<dyn FnMut(raw::c_int) -> bool>;
            let f: &mut (dyn FnMut(raw::c_int) -> bool) = &mut **a;
